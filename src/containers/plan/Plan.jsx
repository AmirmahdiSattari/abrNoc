...
const Plan = () => {
  ...
  useEffect(() => {
    if (selectedRegion) {
      ...
      axios(`https://assignment.abrnoc.com/plans?region=${selectedRegion}`).then(
        (res) => {
          setPlans(res.data);
        }
      ).finally(() => { setIsLoading(false) })
    } else {
      axios(`https://assignment.abrnoc.com/plans?region=Germany`).then((res) => {
        setPlans(res.data);
      }).finally(() => { setIsLoading(false) })
    }
  }, [selectedRegion]);
  ...
  return (
    ...
    flex flex-col items-start py-4 mb-4 mt-11'>
      ...
      <table className='w-[782px]'>
        ...
        {isLoading
          ?
          (<div role="status" className='w-[100%] py-5
